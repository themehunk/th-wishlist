...
class THWL_Settings {
    ...
    /**
     * Build tabs array with icon metadata (backward-compatible with Pro string format).
     */
    private function thwl_get_tabs() {
        $tabs = [
            'general'          => [ 'label' => __( 'General Settings', 'th-wishlist' ),    'icon' => 'dashicons-admin-generic' ],
            'button'           => [ 'label' => __( 'Wishlist Button', 'th-wishlist' ),     'icon' => 'dashicons-heart' ],
            'loop'             => [ 'label' => __( 'Loop Settings', 'th-wishlist' ),       'icon' => 'dashicons-update' ],
            'product'          => [ 'label' => __( 'Product Page', 'th-wishlist' ),        'icon' => 'dashicons-cart' ],
            'wishlist'         => [ 'label' => __( 'Wishlist Page', 'th-wishlist' ),       'icon' => 'dashicons-list-view' ],
            'wishlistRedirect' => [ 'label' => __( 'Wishlist Redirect', 'th-wishlist' ),   'icon' => 'dashicons-randomize' ],
            'style'            => [ 'label' => __( 'Style Customization', 'th-wishlist' ), 'icon' => 'dashicons-admin-appearance' ],
        ];
        return apply_filters( 'thwl_pro_settings_tabs', $tabs );
    }
    ...
    public function settings_page() {
        ...
        $default_columns = apply_filters( 'thwl_default_columns', [
            'thumbnail', 'name', 'price', 'stock', 'add_to_cart', 'remove',
        ] );
        ...
        $tabs = $this->thwl_get_tabs();

        $first_tab_label = '';
        foreach ( $tabs as $tab_data ) {
            $first_tab_label = is_array( $tab_data ) ? $tab_data['label'] : $tab_data;
            break;
        }
        ?>
        <div class="wrap thwl-admin-page">
            <div id="thw-settings-notice"></div>
            <div class="thwl-admin-wrap">

                <!-- ========== SIDEBAR ========== -->
                <div class="thwl-sidebar">
                    <div class="thwl-sidebar-logo">
                    ...
                    <ul class="thwl-nav-list">
                        <?php
                        $first = true;
                        foreach ( $tabs as $id => $tab_data ) :
                            $label = is_array( $tab_data ) ? $tab_data['label'] : $tab_data;
                            $icon  = is_array( $tab_data ) && isset( $tab_data['icon'] ) ? $tab_data['icon'] : 'dashicons-admin-generic';
                        ?>
                        <li class="thwl-nav-item thw-tab <?php echo $first ? 'active' : ''; ?>"
                            data-tab="<?php echo esc_attr( $id ); ?>"
                            data-title="<?php echo esc_attr( $label ); ?>">
                            <span class="dashicons <?php echo esc_attr( $icon ); ?>"></span>
                            <span class="thwl-nav-label"><?php echo esc_html( $label ); ?></span>
                        </li>
                        <?php $first = false; endforeach; ?>

                        <li class="thwl-nav-item thw-tab thwl-help-nav"
                            data-tab="thwl-help"
                            data-title="<?php esc_attr_e( 'Help', 'th-wishlist' ); ?>">
                    ...
                    <form id="thw-settings-form" data-nonce="<?php echo esc_attr( wp_create_nonce( 'thwl_wishlist_nonce' ) ); ?>">
                        ...
                        <div class="thwl-content-header">
                            ...
                            <div class="thwl-header-actions">
                                ...
                                <?php endif; ?>
                                <button type="submit" class="thwl-save-btn">
                                    <?php esc_html_e( 'Save Changes', 'th-wishlist' ); ?>
                                </button>
                            </div>
                        </div>
                                            ...
                                            <ul id="thw-sortable-columns" class="thw-sortable-columns">
                                                ...
                                                foreach ( $saved_columns as $key ) {
                                                    if ( isset( $all_columns[ $key ] ) ) {
                                                        $col_label = isset( $labels[ $key ] ) ? $labels[ $key ] : $all_columns[ $key ];
                                                ?>
                                                <li class="thw-sortable-item">
                                                    ...
                                                    <span class="thw-column-label">
                                                        ...
                                                        <input type="text" name="settings[th_wishlist_table_column_labels][<?php echo esc_attr( $key ); ?>]" value="<?php echo esc_attr( $col_label ); ?>" class="regular-text" />
                                                    </span>
                                                </li>
                                                <?php
                                                    }
                                                }
                                                foreach ( $all_columns as $key => $col_label ) {
                                                    if ( ! in_array( $key, $saved_columns ) ) {
                                                        $custom_label = isset( $labels[ $key ] ) ? $labels[ $key ] : $col_label;
                                                ?>
                                                <li class="thw-sortable-item">
                            ...
                            <div id="thwl-help" class="thw-tab-content">
                                ...
                                <div class="thwl-help-grid">
                                    ...
                                    <div class="thwl-help-card">
                                        ...
                                        <a target="_blank" href="https://themehunk.com/docs/th-wishlist" class="thwl-help-btn"><?php esc_html_e( 'View More', 'th-wishlist' ); ?></a>
                                    </div>
                                    <div class="thwl-help-card">
                                        <span class="dashicons dashicons-sos thwl-help-icon"></span>
                                        <h4><?php esc_html_e( 'Contact Support', 'th-wishlist' ); ?></h4>
                                        <p><?php esc_html_e( 'If you need any help you can contact our support team.', 'th-wishlist' ); ?></p>
                                        <a target="_blank" href="https://themehunk.com/contact-us/" class="thwl-help-btn"><?php esc_html_e( 'Need Help?', 'th-wishlist' ); ?></a>
                                    </div>
                                    <div class="thwl-help-card">
                                        <span class="dashicons dashicons-star-filled thwl-help-icon"></span>
                                        <h4><?php esc_html_e( 'Rate This Plugin', 'th-wishlist' ); ?></h4>
...
@ThemeHunk %20%23WooCommerce%20%23WordPress" class="thwl-help-btn"><?php esc_html_e( 'Click To Tweet', 'th-wishlist' ); ?></a>
                        ...
                        <!-- Hidden reset trigger (fired by sidebar Reset nav item) -->
                        <button type="button" id="thw-reset-settings" class="thwl-hidden-reset"
                            data-nonce="<?php echo esc_attr( wp_create_nonce( 'thwl_wishlist_nonce' ) ); ?>">
                            <?php esc_html_e( 'Reset to Defaults', 'th-wishlist' ); ?>
                        </button>

                    </form>
                </div><!-- .thwl-main-content -->
